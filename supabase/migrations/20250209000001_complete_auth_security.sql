...
-- - account_recovery.password_hash: Argon2id hashed (memory=64MiB, time=3, parallelism=4)
...
BEGIN
  ...
  -- Note: Password verification happens client-side using argon2.verify()
...
-- ✅ Argon2id hashing (memory=64MiB, time=3, parallelism=4) for passwords
-- ✅ CAPTCHA after 3 failed login attempts (client-side)
--
